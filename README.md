...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Badge tin nhắn chưa đọc hình tròn nhỏ đỏ */
        .chat-unread-badge {
            background-color: #ef4444;
            color: white;
            font-size: 11px;
...
<body>
    ...
    <script>
        ...
        let activeChatTargetUser = ''; // Dành riêng cho Admin chọn xem chat của ai
        ...
        /* --- CÁC HÀM XỬ LÝ BOX CHAT REALTIME 2 CHIỀU ĐỘC LẬP (ĐÃ CẬP NHẬT TÍNH NĂNG CHƯA ĐỌC & SẮP XẾP) --- */
        ...
        function renderChatHistory(userKey) {
            ...
            messages.forEach(msg => {
                ...
                if (msg.file) {
                    fileHTML = `
                        <div class="file-tag" style="margin-top: 6px;">
        ...
        function renderUserManagementTable() {
            ...
            registeredUsers.forEach((u, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${u.email || 'N/A'}</td>
                    <td>
                        <select onchange="updateUserKPITypeFromTable('${u.username}', this.value)" style="padding: 4px 8px; border-radius: 4px; border: 1px solid #cbd5e1; outline: none;">
                            <option value="staff" ${(u.kpiType || 'staff') === 'staff' ? 'selected' : ''}>Cán bộ / Nhân viên</option>
                            <option value="leader" ${u.kpiType === 'leader' ? 'selected' : ''}>Lãnh đạo</option>
                            <option value="cleaner" ${u.kpiType === 'cleaner' ? 'selected' : ''}>Lao Công</option>
                        </select>
                    </td>
                    <td>
                        ${u.username !== ADMIN_USERNAME ? `<button class="btn-sm btn-delete" onclick="deleteUser('${u.username}')"><i class="fa-solid fa-trash"></i> Xóa</button>` : '<span style="color:#64748b; font-size:12px;">Admin Root</span>'}
                    </td>
                `;
                tbody.appendChild(tr);
            });
        ...
        function updateUserKPITypeFromTable(username, newType) {
            db.ref(`users/${username}/kpiType`).set(newType, (err) => {
                if (!err) {
                    alert(`Đã cập nhật Bảng KPI cho ${username} thành công!`);
                }
            });
        }
        ...
        function switchTab(tabId, el) {
            ...
            const titles = {
                ...
                'box-chat': 'Box Chat Tin Nhắn Admin'
            };
                ...
                sectionSubs.forEach(sub => {
                    ...
                    subTr.innerHTML = `
                        ...
                        <td colspan="2">
                            ...
                            ${isAdmin ? `<button class="btn-sm btn-edit" style="margin-left: 8px;" onclick="editSubSectionTitle('${sub.id}')"><i class="fa-solid fa-pen"></i> Sửa Tên</button>` : ''}
                        </td>
                        <td style="text-align: center;"><strong>${sub.maxScore}</strong></td>
                        <td style="text-align: center;">-</td>
                        <td style="text-align: center; background-color: #f0fdf4;">-</td>
                        ${isAdmin ? `
                            <td style="text-align: center;">
                                <button class="btn-sm btn-login" style="width: auto; background-color: #2563eb;" onclick="addItemToSubSection('${sub.id}')"><i class="fa-solid fa-plus"></i> Thêm Dòng</button>
                    ...
                    if (sub.items) {
                        sub.items.forEach((item, idx) => {
                            ...
                            const isReadOnlySelf = isAdmin && currentUser !== kpiTargetUser;
                            const isReadOnlyAdmin = !isAdmin;

                            itemTr.innerHTML = `
                                <td style="text-align: center; color: #64748b;">${idx + 1}</td>
                                <td>${item.title}</td>
                                <td><i style="color: #475569;">${item.criteria || ''}</i></td>
                                <td style="text-align: center; font-weight: bold; color: #1e293b;">${item.maxScore}</td>
                                <td style="text-align: center;">
                                    <input type="number" step="0.5" min="0" max="${item.maxScore}" value="${item.selfScore}" 
                                        ${isReadOnlySelf ? 'readonly style="background-color: #f1f5f9;"' : ''} 
                                        onchange="updateKPIScore('${sub.id}', '${item.id}', 'selfScore', this.value)" />
                                </td>
                                <td style="text-align: center; background-color: #f0fdf4;">
                                    <input type="number" step="0.5" min="0" max="${item.maxScore}" value="${item.adminScore}" 
                                        ${isReadOnlyAdmin ? 'readonly style="background-color: #f1f5f9; color: #166534; font-weight: bold;"' : 'style="border-color: #86efac; color: #166534; font-weight: bold;"'} 
                                        onchange="updateKPIScore('${sub.id}', '${item.id}', 'adminScore', this.value)" />
                                </td>
                                ${isAdmin ? `
                                    <td style="text-align: center;">
                                        <button class="btn-sm btn-edit" onclick="editItemTitle('${sub.id}', '${item.id}')"><i class="fa-solid fa-pen"></i> Sửa</button>
        ...
        function updateKPIScore(subId, itemId, field, value) {
            const valNum = parseFloatStrict(value);
            const sub = kpiDataList.find(s => s.id === subId);
            if (sub && sub.items) {
                const item = sub.items.find(i => i.id === itemId);
                if (item) {
                    if (valNum > item.maxScore) {
                        alert(`Điểm nhập (${valNum}) không được vượt quá Điểm tối đa (${item.maxScore})!`);
                        renderKPITable();
                        return;
                    }
                    item[field] = valNum;
        ...
        function calculateKPITotals() {
            ...
            document.getElementById('kpi-total-self').innerText = totalSelf;
            document.getElementById('kpi-total-admin').innerText = totalAdmin;
            document.getElementById('kpi-total-max').innerText = totalMax;
            document.getElementById('kpi-last-time-saved').innerText = lastKpiTimestamp;

            updateKPIRankingDisplay(totalAdmin);
        }

        function updateKPIRankingDisplay(score) {
            const r1 = document.getElementById('rank-check-1');
            const r2 = document.getElementById('rank-check-2');
            const r3 = document.getElementById('rank-check-3');
            const r4 = document.getElementById('rank-check-4');
            const badge = document.getElementById('kpi-ranking-result-display');

            if (!r1 || !r2 || !r3 || !r4 || !badge) return;

            r1.innerHTML = ''; r2.innerHTML = ''; r3.innerHTML = ''; r4.innerHTML = '';

            const checkIcon = '<i class="fa-solid fa-circle-check" style="color: #10b981; font-size: 18px;"></i>';

            if (score >= 90) {
                r1.innerHTML = checkIcon;
                badge.innerText = "Hoàn thành xuất sắc nhiệm vụ";
                badge.className = "ranking-result-badge rank-excel";
            } else if (score >= 75) {
                r2.innerHTML = checkIcon;
                badge.innerText = "Hoàn thành tốt nhiệm vụ";
                badge.className = "ranking-result-badge rank-good";
            } else if (score >= 50) {
                r3.innerHTML = checkIcon;
                badge.innerText = "Hoàn thành nhiệm vụ";
                badge.className = "ranking-result-badge rank-fair";
            } else if (score > 0) {
                r4.innerHTML = checkIcon;
                badge.innerText = "Không hoàn thành nhiệm vụ";
                badge.className = "ranking-result-badge rank-poor";
            } else {
                badge.innerText = "Chưa xếp loại";
                badge.className = "ranking-result-badge";
            }
        }
        ...
        function editSectionTitle(secKey) {
            ...
            const newTitle = prompt(`Nhập Tên Mục Lớn (${secKey}):`, currentTitle);
        ...
        function editSectionMaxScore(secKey) {
            if (!isAdmin) return;
            const currentMax = sectionMaxScores[secKey] || 0;
            const newMax = prompt(`Nhập Điểm Tối Đa cho Mục (${secKey}):`, currentMax);
            if (newMax !== null) {
                const valNum = parseFloatStrict(newMax);
                sectionMaxScores[secKey] = valNum;
                saveTemplateChange();
            }
        }
        ...
        function addMainSection() {
            if (!isAdmin) return;
            const secKey = prompt("Nhập Ký tự đại diện cho Mục Lớn mới (VD: D, E, F...):");
            if (!secKey) return;
            const keyUpper = secKey.trim().toUpperCase();
            if (sectionMaxScores[keyUpper]) { alert("Mục này đã tồn tại!"); return; }

            const secTitle = prompt(`Nhập Tên Mục Lớn (${keyUpper}):`, "Nhiệm vụ bổ sung");
            if (!secTitle) return;

            const maxScoreStr = prompt(`Nhập Điểm tối đa cho Mục (${keyUpper}):`, "20");
            const maxScore = parseFloatStrict(maxScoreStr);

            sectionMaxScores[keyUpper] = maxScore;
            sectionTitles[keyUpper] = secTitle;
            saveTemplateChange();
        }

        function deleteMainSection(secKey) {
            if (!isAdmin) return;
            if (confirm(`Bạn có chắc chắn muốn xóa toàn bộ Mục Lớn (${secKey})?`)) {
        ...
        function addSubSection(secKey) {
            if (!isAdmin) return;
            const code = prompt("Nhập Số La Mã / Mã mục (VD: I, II, III...):", "I");
            if (!code) return;
            const title = prompt("Nhập Tên Mục La Mã:", "Tiêu chí mới");
            if (!title) return;
            const maxScoreStr = prompt("Nhập Điểm Tối Đa cho Mục La Mã này:", "10");
            const maxScore = parseFloatStrict(maxScoreStr);
        ...
        function editSubSectionTitle(subId) {
            ...
            const sub = kpiDataList.find(s => s.id === subId);
            if (!sub) return;
            const newTitle = prompt("Sửa Tên Mục La Mã:", sub.title);
            if (newTitle !== null && newTitle.trim() !== '') {
        ...
        function deleteSubSection(subId) {
            if (!isAdmin) return;
            if (confirm("Bạn có chắc chắn muốn xóa Mục La Mã này và các dòng con?")) {
        ...
        function addItemToSubSection(subId) {
            ...
            const title = prompt("Nhập Nội dung đánh giá dòng mới:", "Nội dung công việc");
            if (!title) return;
            const criteria = prompt("Nhập Tiêu chí đánh giá (nếu có):", "");
            const maxScoreStr = prompt("Nhập Điểm Tối Đa dòng này:", "5");
            const maxScore = parseFloatStrict(maxScoreStr);
        ...
        function editItemTitle(subId, itemId) {
            ...
            if (sub && sub.items) {
                ...
                if (item) {
                    ...
                    if (newTitle !== null) item.title = newTitle.trim();
                    const newCriteria = prompt("Sửa Tiêu chí đánh giá:", item.criteria || '');
                    if (newCriteria !== null) item.criteria = newCriteria.trim();
                    const newMax = prompt("Sửa Điểm tối đa:", item.maxScore);
                    if (newMax !== null) item.maxScore = parseFloatStrict(newMax);
        ...
        function deleteItem(subId, itemId) {
            ...
            const sub = kpiDataList.find(s => s.id === subId);
            if (sub && sub.items) {
                if (confirm("Xóa dòng đánh giá này?")) {
        ...
        function saveTemplateChange() {
            const targetUserInfo = registeredUsers.find(u => u.username === kpiTargetUser);
            const userKpiType = (targetUserInfo && targetUserInfo.kpiType) ? targetUserInfo.kpiType : 'staff';

            masterKPITemplates[userKpiType] = {
                sectionMaxScores: sectionMaxScores,
                sectionTitles: sectionTitles,
                kpiDataList: kpiDataList
            };

            db.ref(`kpiTemplates/${userKpiType}`).set(masterKPITemplates[userKpiType], (err) => {
                if (!err) {
        ...
        function saveKPIRatingByAdmin() {
            ...
            lastKpiTimestamp = `${timeStr} (Bởi Admin: ${currentUser})`;
            saveKPIRatingStorage(lastKpiTimestamp);
            alert(`Đã lưu kết quả chấm KPI của Admin cho tài khoản ${kpiTargetUser} thành công!`);
        }

        function saveKPIRatingByUser() {
            const timeStr = new Date().toLocaleString('vi-VN');
            lastKpiTimestamp = `${timeStr} (Tự chấm)`;
            saveKPIRatingStorage(lastKpiTimestamp);
            alert("Đã lưu bảng điểm tự chấm KPI của bạn thành công!");
        }

        /* =========================================================================
           HÀM XUẤT FILE WORD KPI CHUẨN ĐÚNG THEO YÊU CẦU FILE ĐÍNH KÈM
           ========================================================================= */
        function exportKPIWord() {
            if (typeof docx === 'undefined') {
                alert('Thư viện tạo file Word (docx) chưa tải xong. Vui lòng tải lại trang!');
                return;
            }

            const { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell, AlignmentType, WidthType, BorderStyle, UnderlineType } = docx;
            ...
            else if (userKpiType === 'cleaner') chucVuText = "Lao Công";

            // Xử lý kỳ đánh giá
            let monthYearText = "tháng... năm 2026";
            if (selectedKpiMonth) {
                const parts = selectedKpiMonth.split('-');
                if (parts.length === 2) {
                    monthYearText = `tháng ${parseInt(parts[1])} năm ${parts[0]}`;
                }
            }

            const docChildren = [];

            // 1. PHẦN ĐẦU (Header 2 cột): TRUNG TÂM KSBT / KHOA HÓA LÝ | QUỐC HIỆU TIÊU NGỮ
                            ...
                            new TableCell({
                                ...
                                children: [
                                    new Paragraph({
                                        ...
                                        children: [
                                            new TextRun({ text: "TRUNG TÂM KSBT BẮC NINH", bold: true, size: 22, font: "Times New Roman" })
                                        ]
                                    }),
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [
                                            new TextRun({ text: "KHOA HÓA LÝ", bold: true, underline: { type: UnderlineType.SINGLE }, size: 22, font: "Times New Roman" })
                                        ]
                                    })
                                ]
                            }),
                            new TableCell({
                                ...
                                children: [
                                    ...
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [
                                            new TextRun({ text: "Độc lập - Tự do - Hạnh phúc", bold: true, underline: { type: UnderlineType.SINGLE }, size: 22, font: "Times New Roman" })
                                        ]
                                    })
                                ]
                            })
            ...
            docChildren.push(new Paragraph({ text: "", spaceAfter: 200 }));

            // 2. HỌ VÀ TÊN & CHỨC VỤ
            docChildren.push(new Paragraph({
                children: [
                    new TextRun({ text: "Họ và Tên: ", bold: true, size: 26, font: "Times New Roman" }),
                    new TextRun({ text: (kpiTargetUser || '').toUpperCase(), size: 26, font: "Times New Roman" })
                ],
                spaceAfter: 100
            }));
            ...
            docChildren.push(new Paragraph({
                children: [
                    new TextRun({ text: "Chức vụ: ", bold: true, size: 26, font: "Times New Roman" }),
                    new TextRun({ text: chucVuText, size: 26, font: "Times New Roman" })
                ],
                spaceAfter: 200
            }));

            // 3. TIÊU ĐỀ PHIẾU VÀ KỲ ĐÁNH GIÁ
            docChildren.push(new Paragraph({
                alignment: AlignmentType.CENTER,
                children: [
                    new TextRun({ text: "PHIẾU THEO DÕI ĐÁNH GIÁ VIÊN CHỨC", bold: true, size: 28, font: "Times New Roman" })
                ],
                spaceAfter: 100
            }));
            ...
            docChildren.push(new Paragraph({
                alignment: AlignmentType.CENTER,
                children: [
                    new TextRun({ text: `(Kỳ đánh giá ${monthYearText})`, italics: true, size: 24, font: "Times New Roman" })
                ],
                spaceAfter: 300
            }));

            // 4. BẢNG KPI (Nội dung Đánh giá, Điểm tối đa, Điểm tự chấm, Điểm đánh giá)
            const tableBorders = {
            ...
            kpiRows.push(new TableRow({
                children: [
                    new TableCell({
                        width: { size: 55, type: WidthType.PERCENTAGE },
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Nội Dung Đánh Giá", bold: true, size: 22, font: "Times New Roman" })] })]
                    }),
                    new TableCell({
                        width: { size: 15, type: WidthType.PERCENTAGE },
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Điểm tối đa", bold: true, size: 22, font: "Times New Roman" })] })]
                    }),
                    new TableCell({
                        width: { size: 15, type: WidthType.PERCENTAGE },
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Điểm tự chấm", bold: true, size: 22, font: "Times New Roman" })] })]
                    }),
                    new TableCell({
                        width: { size: 15, type: WidthType.PERCENTAGE },
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Điểm đánh giá", bold: true, size: 22, font: "Times New Roman" })] })]
                    })
                ]
            }));

            let grandSelfTotal = 0;
            let grandAdminTotal = 0;

            Object.keys(sectionMaxScores).forEach(secKey => {
                const secTitle = sectionTitles[secKey] || '';
                const secMax = sectionMaxScores[secKey] || 0;

                // Dòng Mục Lớn (A, B, C...)
                kpiRows.push(new TableRow({
                    children: [
                        new TableCell({
                            children: [new Paragraph({ children: [new TextRun({ text: `${secKey}. ${secTitle}`, bold: true, size: 22, font: "Times New Roman" })] })]
                        }),
                        new TableCell({
                            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${secMax}`, bold: true, size: 22, font: "Times New Roman" })] })]
                        }),
                ...
                sectionSubs.forEach(sub => {
                    ...
                    kpiRows.push(new TableRow({
                        children: [
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun({ text: `${sub.code}. ${sub.title}`, bold: true, size: 22, font: "Times New Roman" })] })]
                            }),
                            new TableCell({
                                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${sub.maxScore}`, bold: true, size: 22, font: "Times New Roman" })] })]
                            }),
                            new TableCell({ children: [new Paragraph({ text: "" })] }),
                            new TableCell({ children: [new Paragraph({ text: "" })] })
                        ]
                    ...
                    if (sub.items) {
                        sub.items.forEach(item => {
                            ...
                            grandSelfTotal += selfSc;
                            grandAdminTotal += adminSc;

                            let itemText = item.title;
                            if (item.criteria) {
                                itemText += ` (${item.criteria})`;
                            }

                            kpiRows.push(new TableRow({
                                children: [
                                    new TableCell({
                                        children: [new Paragraph({ children: [new TextRun({ text: itemText, size: 22, font: "Times New Roman" })] })]
                                    }),
                                    new TableCell({
                                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${item.maxScore}`, size: 22, font: "Times New Roman" })] })]
                                    }),
                                    new TableCell({
                                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${selfSc}`, size: 22, font: "Times New Roman" })] })]
                                    }),
                                    new TableCell({
                                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${adminSc}`, size: 22, font: "Times New Roman" })] })]
                                    })
                                ]
                            }));
                        });
            ...
            // Dòng Tổng điểm đánh giá
            kpiRows.push(new TableRow({
                children: [
                    new TableCell({
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "TỔNG ĐIỂM ĐÁNH GIÁ", bold: true, size: 22, font: "Times New Roman" })] })]
                    }),
                    new TableCell({ children: [new Paragraph({ text: "" })] }),
                    new TableCell({
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${grandSelfTotal}`, bold: true, size: 22, font: "Times New Roman" })] })]
                    }),
                    new TableCell({
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `${grandAdminTotal}`, bold: true, size: 22, font: "Times New Roman" })] })]
                    })
                ]
            }));

            const kpiMainTable = new Table({
                width: { size: 100, type: WidthType.PERCENTAGE },
                borders: tableBorders,
                rows: kpiRows
            });

            docChildren.push(kpiMainTable);
            docChildren.push(new Paragraph({ text: "", spaceAfter: 250 }));

            // 5. BẢNG XẾP LOẠI (Lấy dữ liệu từ bảng xếp loại trên web)
            const rankingRows = [
                new TableRow({
                    children: [
                        new TableCell({
                            width: { size: 15, type: WidthType.PERCENTAGE },
                            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "STT", bold: true, size: 22, font: "Times New Roman" })] })]
                        }),
                        new TableCell({
                            width: { size: 40, type: WidthType.PERCENTAGE },
                            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Mức xếp loại", bold: true, size: 22, font: "Times New Roman" })] })]
                        }),
                        new TableCell({
                            width: { size: 30, type: WidthType.PERCENTAGE },
                            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Mức điểm", bold: true, size: 22, font: "Times New Roman" })] })]
                        }),
                        new TableCell({
                            width: { size: 15, type: WidthType.PERCENTAGE },
                            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Đạt được", bold: true, size: 22, font: "Times New Roman" })] })]
                        })
                    ]
                })
            ];
            ...
            const rankDefs = [
                { stt: "1", label: "Hoàn thành xuất sắc nhiệm vụ", scoreRange: "Từ 90 điểm trở lên", isMatch: grandAdminTotal >= 90 },
                { stt: "2", label: "Hoàn thành tốt nhiệm vụ", scoreRange: "Từ 75 đến dưới 90 điểm", isMatch: grandAdminTotal >= 75 && grandAdminTotal < 90 },
                { stt: "3", label: "Hoàn thành nhiệm vụ", scoreRange: "Từ 50 đến dưới 75 điểm", isMatch: grandAdminTotal >= 50 && grandAdminTotal < 75 },
                { stt: "4", label: "Không hoàn thành nhiệm vụ", scoreRange: "Dưới 50 điểm", isMatch: grandAdminTotal > 0 && grandAdminTotal < 50 }
            ];

            let finalRankText = "Chưa xếp loại";

            rankDefs.forEach(r => {
                if (r.isMatch) finalRankText = r.label;
                rankingRows.push(new TableRow({
                    children: [
                        new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: r.stt, size: 22, font: "Times New Roman" })] })] }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: r.label, bold: true, size: 22, font: "Times New Roman" })] })] }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: r.scoreRange, size: 22, font: "Times New Roman" })] })] }),
                        new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: r.isMatch ? "x" : "", bold: true, size: 22, font: "Times New Roman" })] })] })
                    ]
                }));
            });
            ...
            docChildren.push(rankingTable);
            docChildren.push(new Paragraph({ text: "", spaceAfter: 150 }));

            // 6. DÒNG KẾT QUẢ XẾP LOẠI (Dưới bảng và ở 1 dòng riêng)
            docChildren.push(new Paragraph({
                children: [
                    new TextRun({ text: "Kết quả xếp loại: ", bold: true, size: 24, font: "Times New Roman" }),
                    new TextRun({ text: finalRankText, size: 24, font: "Times New Roman" })
                ],
                spaceAfter: 300
            }));

            // 7. PHẦN CHỮ KÝ CĂN LỆCH VỀ BÊN PHẢI
                            ...
                            new TableCell({
                                ...
                                children: [
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [
                                            new TextRun({ text: "Bắc Ninh, ngày....tháng....năm 2026", italics: true, size: 22, font: "Times New Roman" })
                                        ]
                                    }),
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [
                                            new TextRun({ text: "XÁC NHẬN CỦA LÃNH ĐẠO KHOA, TRƯỞNG KHOA", bold: true, size: 22, font: "Times New Roman" })
                                        ]
                                    })
                                ]
                            })
            ...
            const doc = new Document({
                sections: [{
                    properties: {
                        page: {
                            margin: { top: 1134, bottom: 1134, left: 1134, right: 1134 } // 2cm margin
                        }
                    },
            ...
            // Xuất và tải file Word
            Packer.toBlob(doc).then(blob => {
                const fileName = `Phieu_Danh_Gia_KPI_${kpiTargetUser || 'CanBo'}_${selectedKpiMonth}.docx`;
